...
class AutopoolTen extends React.Component {
   ...
   async componentDidMount(){
       ...
       await axios.get('/api/Admin/getPoolTenDetails')
        .then(res => {
       ...
       await axios.get('/api/Admin/getPoolTenAvailableDetails')
        .then(res => {
        ...
        await axios.get('/api/Admin/getPoolTenCompletedDetails')
        .then(res => {
   ...
   { members &&   members.map(Direct => {
    ...
    handleDelete = async () => {
      ...
      for (let i = 0; i < DeleteArray.length; i++) {
       ...
       await axios.post('/api/Admin/performDeleteTen',{

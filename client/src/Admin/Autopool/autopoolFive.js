...
class AutopoolFive extends React.Component {
   ...
   async componentDidMount(){
       ...
       await axios.get('/api/Admin/getPoolFiveDetails')
        .then(res => {
       ...
       await axios.get('/api/Admin/getPoolFiveAvailableDetails')
        .then(res => {
        ...
        await axios.get('/api/Admin/getPoolFiveCompletedDetails')
        .then(res => {
  ...
  createDeleteTable = (members)=> {
   ...
   { members &&   members.map(Direct => {
    ...
    handleInitializeAutopool = async () =>{
      ...
      await  axios.post('/api/Admin/InitialisedAutopoolFive',{
    ...
    handleDelete = async () => {
      ...
      for (let i = 0; i < DeleteArray.length; i++) {
       ...
       await axios.post('/api/Admin/performDeleteFive',{

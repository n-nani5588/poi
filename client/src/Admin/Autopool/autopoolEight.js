...
class Autopooleight extends React.Component {
   ...
   async componentDidMount(){
       ...
       await axios.get('/api/Admin/getPoolEightDetails')
        .then(res => {
       ...
       await axios.get('/api/Admin/getPoolEightAvailableDetails')
        .then(res => {
        ...
        await axios.get('/api/Admin/getPoolEightCompletedDetails')
        .then(res => {
  ...
  createDeleteTable = (members)=> {
   ...
   { members &&   members.map(Direct => {
    ...
    handleInitializeAutopool = async () =>{
      ...
      await  axios.post('/api/Admin/InitialisedAutopoolEight',{
    ...
    handleDelete = async () => {
      ...
      for (let i = 0; i < DeleteArray.length; i++) {
       ...
       await axios.post('/api/Admin/performDeleteEight',{

...
class AutopoolSix extends React.Component {
   ...
   async componentDidMount(){
       ...
       await axios.get('/api/Admin/getPoolSixDetails')
        .then(res => {
       ...
       await axios.get('/api/Admin/getPoolSixAvailableDetails')
        .then(res => {
        ...
        await axios.get('/api/Admin/getPoolSixCompletedDetails')
        .then(res => {
  ...
  createDeleteTable = (members)=> {
   ...
   { members &&   members.map(Direct => {
    ...
    handleInitializeAutopool = async () =>{
      ...
      await  axios.post('/api/Admin/InitialisedAutopoolSix',{
    ...
    handleDelete = async () => {
      ...
      for (let i = 0; i < DeleteArray.length; i++) {
       ...
       await axios.post('/api/Admin/performDeleteSix',{

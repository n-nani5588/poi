...
class AutopoolNine extends React.Component {
   ...
   async componentDidMount(){
       ...
       await axios.get('/api/Admin/getPoolNineDetails')
        .then(res => {
       ...
       await axios.get('/api/Admin/getPoolNineAvailableDetails')
        .then(res => {
        ...
        await axios.get('/api/Admin/getPoolNineCompletedDetails')
        .then(res => {
  ...
  createDeleteTable = (members)=> {
   ...
   { members &&   members.map(Direct => {
    ...
    handleInitializeAutopool = async () =>{
      ...
      await  axios.post('/api/Admin/InitialisedAutopoolNine',{
    ...
    handleDelete = async () => {
      ...
      for (let i = 0; i < DeleteArray.length; i++) {
       ...
       await axios.post('/api/Admin/performDeleteNine',{
